// Reveal declarativo de grupos en scroll (portado de ZEGM_web). data-reveal-group en
// el contenedor → sus hijos directos revelan escalonados (autoAlpha + y, power4.inOut).
// Soporta grupos anidados (data-reveal-group-nested) y attrs data-stagger/data-distance/
// data-start. Mismo start/clamp que motion.ts y split-text.ts para sincronizar todo.
...
export function initRevealGroup(root: HTMLElement): void {
  root.querySelectorAll<HTMLElement>('[data-reveal-group]').forEach((groupEl) => {
    ...
    // Modo mount: revela al montar (sin ScrollTrigger). Para contenido above-the-fold cuyo
    // trigger no dispara fiable al cargar — mismo criterio que data-aa-split="mount".
    ...
    if (!directChildren.length) {
      ...
      if (onMount) playBlock();
      else ScrollTrigger.create({ trigger: groupEl, start: triggerStart, once: true, onEnter: playBlock });
      return;
    }
